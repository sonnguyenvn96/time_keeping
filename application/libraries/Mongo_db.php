...
Class Mongo_db{
	...
	*/
    ...
    public function getID($name) {
			...
			return $run[0]['value']->id;
		}
		
    /**
     * Tên hàm getDataFaster
     *
     * Hàm update và lấy ra bản ghi tốc độ nhanh
     *
     * @param (array) ($where_array) array('fieldname' => value)
     * @param (array) ($array_modify) array('fieldname' => value)
     * @param (bool) ($update) true or false, nếu true thì là update, nếu false thì là tăng dần
     * @param (string) ($_table) tên bảng
     * @param (string) ($field_order) trường muốn order
     * @param (bool) ($sort) kiểu sắp xếp lấy ra
     * @return (array)
     */
    public function findAndModify($where_array, $array_modify, $_table, $sort = true, $update, $field_order) {
		$action = $update ? '$set' : '$inc';
		$command = array(
			'findAndModify' => $_table,
			'query' => $where_array,
			'update' => array($action => $array_modify),
			'new' => true,
			'sort' => array($field_order => ($sort ? 1 : 0))
		);
		
		$run = $this->command($command);
		if($run[0]['ok'] == 1)
		{
			return (array)$run[0]['value'];
		}
		return [];
    }
	...
	*/
	...
	public function increase($fields, $value = NULL) {
        $this->_u('$inc');
        if (is_string($fields)) {
	...
	* Removes by an array by the value of a field
	*
	* @usage: $this->mongo_db->pullAll('comments', array('comment_id'=>123))->update('blog_posts');
	*/
	public function pullAll($field = "", $value = array())
	{
		$this->_u('$pullAll');
	...
	*/
	public function aggregate($collection, $operation)
	{
		...
		$command = array('aggregate'=>$collection, 'pipeline'=>$operation, 'cursor' => (object)array());
